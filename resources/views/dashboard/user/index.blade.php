...
<x-app-layout title="Mijn Dashboard">
    <header class="flex flex-col md:flex-row justify-between items-center md:items-end mb-16 gap-8">
        <div class="space-y-6 w-full flex flex-col items-center md:items-start text-center md:text-left">
            <h1 class="text-5xl md:text-6xl font-black text-white tracking-tighter uppercase italic">
                Mijn <span class="text-gym-primary">Artikelen</span>
            </h1>
        </div>
    ...
    @if (session('success'))
        <div role="alert"
             class="alert alert-success mb-8 text-white font-bold border-none bg-green-600/90 backdrop-blur-sm">
            <i data-lucide="check-circle" class="w-6 h-6"></i>
            <span>{{ session('success') }}</span>
        </div>
    @endif

    {{-- Articles Grid with delete and edit options --}}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        @forelse($posts as $post)
            <x-shared.card :$post :show-actions="true" />
        @empty
